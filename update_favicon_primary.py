...
# Define primary button color (HSL 24 95% 53% converted to RGB)
# This is the same orange as the Run Code button
primary_r, primary_g, primary_b = 248, 105, 22

# Replace green pixels with primary orange color
for y in range(img_array.shape[0]):
    for x in range(img_array.shape[1]):
        if green_mask[y, x]:
            brightness = g[y, x] / 255.0
            img_array[y, x, 0] = int(primary_r * brightness)
            img_array[y, x, 1] = int(primary_g * brightness)
            img_array[y, x, 2] = int(primary_b * brightness)
...
# Save the orange favicon
favicon.save('client/public/favicon.png', 'PNG')
print("✅ Favicon updated with primary theme color:")
print("   - Orange color matching Run Code button")
print("   - Large crab with thick lines maintained")
print("   - Production-ready branding")